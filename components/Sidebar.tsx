...
export function Sidebar({ clinica, ciudad, logoUrl }: { clinica: string; ciudad: string; logoUrl: string | null }) {
  const pathname = usePathname()
  const { data: session } = useSession()

  return (
    <aside className="w-64 min-h-screen bg-slate-900 flex flex-col fixed left-0 top-0 bottom-0 z-50">
      {/* Logo */}
      <div className="p-5 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-white font-bold text-sm leading-tight truncate">{clinica}</p>
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        <p className="text-slate-500 text-xs font-medium uppercase tracking-wider px-3 py-2 mt-1">Menú principal</p>
        {navItems.map((item) => {
