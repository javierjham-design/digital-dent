...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await prisma.seccionPlan.update({ where: { id }, data })
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Tratamientos vinculados quedan con seccionId=null (onDelete: SetNull)
  await prisma.seccionPlan.delete({ where: { id } })
