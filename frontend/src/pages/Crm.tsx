...
function AgendarLeadModal({ lead, navigate, notify, onClose, onDone }: {
  ...
  // Disponibilidad real: slots libres del profesional (del más cercano al más lejano).
  ...
  useEffect(() => {
    ...
    crmService.disponibilidad(doctorId, duracion, 30)
      .then((r) => { if (vivo) setSlots(r.slots) })
  ...
  const gruposSlots = useMemo(() => {
    const m = new Map<string, { iso: string; hora: string }[]>()
    for (const s of slots) {
      const d = new Date(s.start)
      const dia = d.toLocaleDateString('es-CL', { timeZone: TZ, weekday: 'short', day: 'numeric', month: 'short' })
      const hora = d.toLocaleTimeString('es-CL', { timeZone: TZ, hour: '2-digit', minute: '2-digit', hour12: false })
      if (!m.has(dia)) m.set(dia, [])
      m.get(dia)!.push({ iso: s.start, hora })
    }
    return [...m.entries()]
  }, [slots])
