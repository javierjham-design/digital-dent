...
interface Plan { id: string; nombre: string; descripcion: string | null; precioMensual: number; maxProfesionales: number; destacado: boolean; activo: boolean; orden: number }
const fmtCLP = (n: number) => new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(n)

export function AdminPlanes() {
  const [planes, setPlanes] = useState<Plan[]>([])
  const [form, setForm] = useState({ id: '', nombre: '', precioMensual: '', maxProfesionales: '2' })
  ...
  async function crear() {
    try {
      await adminService.crearPlan({ id: form.id, nombre: form.nombre, precioMensual: Number(form.precioMensual), maxProfesionales: Number(form.maxProfesionales) || 2 })
      setForm({ id: '', nombre: '', precioMensual: '', maxProfesionales: '2' }); setMsg(''); cargar()
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-x-auto mb-6">
        <table className="w-full text-sm min-w-[520px]">
          <thead><tr className="border-b border-slate-800 text-xs uppercase tracking-wider text-slate-500">
            <th className="text-left px-6 py-3">Plan</th><th className="text-left px-6 py-3">Precio mensual</th><th className="text-left px-6 py-3">Profesionales</th><th className="text-left px-6 py-3">Estado</th><th className="px-6 py-3"></th>
          </tr></thead>
