...
type PermisoCampo = 'puedeGestionarCrm' | 'puedeGestionarLiquidaciones' | 'puedeEliminar' | 'puedeGestionarCajas' | 'puedeRecibirPagos' | 'puedeConfigurarClinica' | 'puedeGestionarEquipo'
  ...
  return async (req: Request, _res: Response, next: NextFunction) => {
    try {
      ...
      const u = await tenantDb(req).user.findUnique({
        ...
        select: { puedeGestionarCrm: true, puedeGestionarLiquidaciones: true, puedeEliminar: true, puedeGestionarCajas: true, puedeRecibirPagos: true, puedeConfigurarClinica: true, puedeGestionarEquipo: true },
      })
