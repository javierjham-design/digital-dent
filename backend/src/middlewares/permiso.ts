...
type PermisoCampo = 'puedeGestionarCrm' | 'puedeGestionarLiquidaciones' | 'puedeEliminar'
  ...
  return async (req: Request, _res: Response, next: NextFunction) => {
    try {
      ...
      const u = await tenantDb(req).user.findUnique({
        ...
        select: { puedeGestionarCrm: true, puedeGestionarLiquidaciones: true, puedeEliminar: true },
      })
