...
function buildPasos(_termino: string, lugar: string) {
  return [
    { n: '1', t: `Crea tu ${lugar}`, d: 'En minutos tienes tu espacio con tu equipo, horarios y catálogo de prestaciones.' },
    { n: '2', t: 'Carga tu agenda', d: `Agenda, confirma por WhatsApp y lleva la ficha de cada uno.` },
...
export function Landing() {
  const rubroInicial = getVertical(new URLSearchParams(window.location.search).get('rubro')).id
  const [rubro, setRubro] = useState<VerticalId>(rubroInicial)
  const [planes, setPlanes] = useState<PlanLanding[]>([])
  const [anual, setAnual] = useState(false)
  const [faqAbierta, setFaqAbierta] = useState<number | null>(0)
  const [demoOpen, setDemoOpen] = useState(false)

  useEffect(() => { fetchPlanes().then(setPlanes).catch(() => setPlanes([])) }, [])

  const desde = planes[0]?.precioMensual ?? 39900
    ...
    <div className="min-h-screen bg-white text-slate-900 antialiased">
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href={appLoginUrl()} className="hidden sm:inline-flex px-3.5 py-2 text-sm font-semibold text-slate-700 hover:text-slate-900 transition-colors">Iniciar sesión</a>
            <button onClick={() => setDemoOpen(true)} className="px-4 py-2 text-sm font-semibold text-white bg-cyan-600 hover:bg-cyan-700 rounded-xl shadow-sm transition-colors">Probar demo gratis</button>
          </div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-5 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
          <div>
              ...
              <div className="inline-flex bg-slate-100 rounded-xl p-1 mb-5">
                {VERTICAL_IDS.map((id) => (
                  <button key={id} onClick={() => setRubro(id)}
                    className={`px-3.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all ${rubro === id ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            <Reveal delay={120}><p className="text-lg text-slate-600 leading-relaxed mt-5 max-w-xl">{v.subtitle}</p></Reveal>
            <Reveal delay={180}>
              <div className="flex flex-col sm:flex-row gap-3 mt-8">
                <button onClick={() => setDemoOpen(true)} className="px-6 py-3.5 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold rounded-xl shadow-lg shadow-cyan-600/20 transition-all hover:-translate-y-0.5">Probar la demo en vivo →</button>
                <a href="#planes" className="px-6 py-3.5 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 font-semibold rounded-xl transition-colors text-center">Ver planes</a>
              </div>
            </Reveal>
            <Reveal delay={240}>
          ...
          <Reveal delay={200}><AgendaMockup /></Reveal>
        </div>
      </section>
            ...
            <Reveal key={f.t} delay={(i % 3) * 80}>
              <div className="h-full bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md hover:border-cyan-200 transition-all">
                <div className="w-11 h-11 rounded-xl bg-cyan-50 flex items-center justify-center mb-4">
                  <svg className="w-5 h-5 text-cyan-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={f.icon} /></svg>
                </div>
      ...
      <section id="planes" className="max-w-6xl mx-auto px-5 py-20">
        ...
        <Reveal className="flex items-center justify-center gap-3 mb-10">
          ...
          <button onClick={() => setAnual((x) => !x)} className="relative w-12 h-6 rounded-full bg-cyan-600 transition-colors" aria-label="Cambiar a facturación anual">
            <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${anual ? 'left-6' : 'left-0.5'}`} />
          </button>
          <span className={`text-sm font-medium ${anual ? 'text-slate-900' : 'text-slate-400'}`}>Anual <span className="text-cyan-600 font-semibold">(2 meses gratis)</span></span>
        </Reveal>
        <div className="grid md:grid-cols-3 gap-6 items-start">
          {planes.map((p, i) => {
            const mensual = anual ? Math.round((p.precioAnual ?? p.precioMensual * 10) / 12) : p.precioMensual
            return (
              <Reveal key={p.id} delay={i * 80}>
                <div className={`relative rounded-2xl p-7 h-full flex flex-col ${p.destacado ? 'bg-slate-900 text-white border-2 border-cyan-500 shadow-xl' : 'bg-white text-slate-900 border border-slate-200 shadow-sm'}`}>
                  {p.destacado && <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-cyan-500 text-white text-xs font-bold rounded-full uppercase tracking-wide">Más popular</span>}
                  ...
                  <p className="text-xs text-slate-400">{anual ? 'facturado anual · IVA incl.' : 'facturación mensual · IVA incl.'}</p>
        ...
        <Reveal className="mt-8">
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-start gap-3 max-w-3xl mx-auto">
            <svg className="w-6 h-6 text-emerald-600 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884" /></svg>
            <div>
              <p className="font-semibold text-emerald-900">Confirmaciones por WhatsApp — servicio adicional</p>
              <p className="text-sm text-emerald-700 mt-0.5">Recordatorios automáticos que confirman, reagendan o cancelan citas solas. Se cobra aparte según el volumen de tu clínica. Lo activamos cuando quieras.</p>
            </div>
          </div>
        </Reveal>
      </section>
      ...
      <section className="bg-slate-50 border-y border-slate-100">
        <div className="max-w-6xl mx-auto px-5 py-20">
          <Reveal className="text-center max-w-2xl mx-auto mb-14"><h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Lo que dicen quienes ya lo usan</h2></Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIOS.map((t, i) => (
              <Reveal key={t.n} delay={i * 90}>
                <figure className="bg-white rounded-2xl p-7 border border-slate-100 h-full flex flex-col">
                  <div className="flex gap-0.5 mb-4 text-amber-400">
                    {Array.from({ length: 5 }).map((_, k) => (<svg key={k} className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path d="M9.05 2.93c.3-.92 1.6-.92 1.9 0l1.36 4.18a1 1 0 00.95.69h4.4c.97 0 1.37 1.24.59 1.81l-3.56 2.59a1 1 0 00-.36 1.12l1.36 4.18c.3.92-.76 1.69-1.54 1.12l-3.56-2.59a1 1 0 00-1.18 0l-3.56 2.59c-.78.57-1.84-.2-1.54-1.12l1.36-4.18a1 1 0 00-.36-1.12L1.4 9.61c-.78-.57-.38-1.81.59-1.81h4.4a1 1 0 00.95-.69L9.05 2.93z" /></svg>))}
                  </div>
      ...
      <section id="faq" className="max-w-3xl mx-auto px-5 py-20">
        <Reveal className="text-center mb-12"><h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Preguntas frecuentes</h2></Reveal>
        <div className="space-y-3">
          {FAQS.map((f, i) => (
            <Reveal key={f.q} delay={i * 50}>
              <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
                <button onClick={() => setFaqAbierta(faqAbierta === i ? null : i)} className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left">
                  <span className="font-semibold text-slate-900">{f.q}</span>
                  <svg className={`w-5 h-5 text-slate-400 flex-shrink-0 transition-transform ${faqAbierta === i ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </button>
                {faqAbierta === i && <p className="px-5 pb-5 text-sm text-slate-600 leading-relaxed">{f.a}</p>}
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="max-w-6xl mx-auto px-5 pb-20">
        <Reveal>
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-cyan-600 to-teal-700 px-8 py-14 text-center">
            ...
            <p className="text-cyan-50 text-lg mt-4 max-w-xl mx-auto relative">Creamos una clínica de demostración con pacientes y citas ficticias para que la recorras sin compromiso.</p>
            <button onClick={() => setDemoOpen(true)} className="relative mt-8 px-7 py-3.5 bg-white text-cyan-700 font-bold rounded-xl shadow-lg hover:-translate-y-0.5 transition-all">Crear mi demo gratis →</button>
          </div>
        </Reveal>
      </section>
...
export function Check({ className = 'text-cyan-600' }: { className?: string }) {
  return (<svg className={`w-4 h-4 flex-shrink-0 ${className}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>)
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl shadow-slate-300/40 overflow-hidden rotate-1 hover:rotate-0 transition-transform duration-500">
      <div className="bg-slate-50 border-b border-slate-100 px-4 py-3 flex items-center gap-2">
        <div className="flex gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-rose-300" /><span className="w-2.5 h-2.5 rounded-full bg-amber-300" /><span className="w-2.5 h-2.5 rounded-full bg-emerald-300" /></div>
      ...
      <div className="divide-y divide-slate-50">
        {rows.map((r) => (
          <div key={r.h} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-slate-800 truncate">{r.n}</p></div>
...
export function DemoModal({ vertical, onClose }: { vertical: VerticalId; onClose: () => void }) {
  ...
  const [cred, setCred] = useState<{ slug: string; usuario: string; password: string; url: string | null } | null>(null)
  ...
  async function crear(e: React.FormEvent) {
    ...
    try {
      ...
      setEstado('listo')
      // Handoff de sesión por subdominio (cross-origin): la SPA lee #token.
      if (url) window.location.href = url
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo crear la demo. Intenta nuevamente.')
      setEstado('error')
    }
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        {estado === 'listo' && cred ? (
          <div className="p-7 text-center">
            ...
            <p className="text-sm text-slate-600 mt-2">{cred.url ? 'Te estamos llevando a tu clínica de prueba…' : 'Entra con estas credenciales.'}</p>
            <div className="mt-5 bg-slate-50 border border-slate-200 rounded-xl p-4 text-left text-sm">
              ...
              <p className="text-slate-700">Clínica: <span className="font-mono font-semibold">{cred.slug}</span></p>
            ...
            {cred.url && <a href={cred.url} className="mt-5 inline-block w-full py-3 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold rounded-xl transition-colors">Entrar a la demo →</a>}
          </div>
        ) : (
          <form onSubmit={crear} className="p-7">
            <div className="flex items-start justify-between mb-1">
              <h3 className="text-xl font-bold text-slate-900">Prueba Cláriva gratis</h3>
              <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 -mt-1"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            ...
            <button type="submit" disabled={estado === 'creando'} className="mt-5 w-full py-3 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-60 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
              {estado === 'creando' ? (<><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.37 0 0 5.37 0 12h4z" /></svg>Creando tu clínica de prueba…</>) : 'Crear mi demo →'}
            </button>
